...
class SalesContract implements IContract {
  ...
  describe() {
    console.log('------------------------');
    console.log(`Contract: ${this.subject}`);
    this.clauses.forEach((c) => { console.log(`Clause: ${c}`); });
    this.signatories.forEach((s) => { console.log(`Signed by: ${s.name}`); });
    console.log('------------------------\n');
  }
}
